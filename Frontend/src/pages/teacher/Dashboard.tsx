...
export default function TeacherDashboard() {
  const { navigate, token, user } = useApp()
  const [profileReviews, setProfileReviews] = useState<{ _id: string; text: string; rating: number }[]>([])
  const [feedbackError, setFeedbackError] = useState('')
  useEffect(() => {
    if (!token || !user) return
    api<{ reviews: { _id: string; text: string; rating: number }[] }>('/professors/' + user._id + '/feedback', {}, token)
      .then(data => setProfileReviews(data.reviews)).catch(error => setFeedbackError(error.message))
  }, [token, user])
  ...
  return <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
    ...
    <Card className="mt-6 p-6"><h2 className="font-semibold mb-4">Reviews on my teacher profile</h2>{feedbackError && <p role="alert" className="text-sm text-danger">{feedbackError}</p>}{profileReviews.map(review => <div key={review._id} className="border-b border-line py-3"><p className="text-sm font-semibold">{review.rating} / 5</p><p className="text-sm">{review.text}</p></div>)}{!profileReviews.length && !feedbackError && <EmptyState title="No profile reviews yet" />}</Card>
